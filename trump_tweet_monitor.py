...
PUSHPLUS_TOPIC = os.getenv('PUSHPLUS_TOPIC', '')  # 群组编码
...
def send_to_pushplus(text, title="特朗普推文监控"):
    """发送消息到 PushPlus"""
    data = {
        "token": PUSHPLUS_TOKEN,
        "title": title,
        "content": text,
        "topic": PUSHPLUS_TOPIC,  # 群组编码，发送给群组内所有成员
        "template": "txt"  # 使用纯文本模板
    }
    try:
        # PushPlus 不需要代理，直接发送
        response = requests.post(PUSHPLUS_API_URL, json=data)
        result = response.json()
        
        if result.get('code') == 200:
            logging.info(f"消息发送成功，流水号: {result.get('data')}")
            print(f"消息发送成功，流水号: {result.get('data')}")
            return True
        else:
...
def push_recent_tweets(count=3):
    ...
    if not feed.entries:
        ...
        if not feed.entries:
            ...
            send_to_pushplus("⚠️ 未能获取到特朗普的最近推文，所有RSS源均无返回")
            return None
    else:
    ...
    # 发送汇总消息
    send_to_pushplus(f"📋 特朗普最近{len(entries)}条推文汇总：")
    
    # 逐条发送
    for entry in entries:
        send_to_pushplus(format_tweet_message(entry))
        ...
        try:
            ...
            if hasattr(feed, 'entries') and feed.entries:
                ...
                if last_entry_id != latest_entry.id:
                    ...
                    print(new_tweet_msg)
                    send_to_pushplus(format_tweet_message(latest_entry))
